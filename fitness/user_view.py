...
def user(exercise, sensitivity):
    poses_dir = "./poses"
    guide_image, landmarks_of_interest, connections, guide_pose = None, None, None, None

    def update_from_pose(path):
        with open(f"{path}.json", "r") as f:
            data = json.load(f)
        nonlocal guide_image, landmarks_of_interest, connections, guide_pose
    ...
    poses_to_complete = natsorted(
        [f"{poses_dir}/{exercise}/{item.split('.')[0]}" for item in os.listdir(f"{poses_dir}/{exercise}") if
         item.endswith(".json")])
    current_exercise = 0
    ...
    def calculate_total_similarity(current_pose, saved_pose):
        feedback = []
        direction_scores = [calculate_direction_similarity(start, end, current_pose, saved_pose, feedback) for
    ...
    while cap.isOpened():
        ...
        if show_guide and guide_image is not None:
            # Resize guide_image to match frame size
            guide_image = cv2.resize(guide_image, (frame.shape[1], frame.shape[0]))

            # Ensure guide_image is 3-channel (BGR)
            if len(guide_image.shape) == 2:
                guide_image = cv2.cvtColor(guide_image, cv2.COLOR_GRAY2BGR)
            ...
            for (start, end) in connections:
                if start in guide_pose and end in guide_pose:
        ...
        cv2.putText(frame, f"Similarity Score S: {similarity_score_s:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (0, 0, 255), 2)
